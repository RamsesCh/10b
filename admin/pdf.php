<?php
	...
	$pdf->Cell(180,20,utf8_decode('Resultados por nivel académico '.$nivel),1,1, 'C',1);
	$pdf->Cell(180,10,utf8_decode('El reporte se realizó en el periodo de "'.$fecha_inicio.'" al "'.$fecha_fin.'"'),1,1, 'C');
	
	$pdf->Cell(180,6,' ',0,1, 'C');
	$pdf->Cell(180,10,'ANSIEDAD',1,1, 'C', 1);
	$pdf->Cell(65,6,'Escala Ansiedad de Estado',1,0, 'C',1);
	...
	AND '$fecha_fin' AND U.Nivel = '$nivel' GROUP BY Estado";
	...
	WHERE D.Fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' AND U.Nivel = '$nivel' GROUP BY Rango";
	$resultado2 = $mysqli->query($query2);

	$pdf->Cell(180,6,' ',0,1, 'C');
	$pdf->Cell(180,10,utf8_decode('DEPRESIÓN'),1,1, 'C', 1);
	...
	WHERE M.Fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' AND U.Nivel = '$nivel' GROUP BY Cantidad_profunda";
	$resultado3 = $mysqli->query($query3);

	$pdf->Cell(180,6,' ',0,1, 'C');
	$pdf->Cell(180,10,utf8_decode('MOTIVACIÓN ACADÉMICA'),1,1, 'C', 1);
	...
	WHERE E.Fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' AND U.Nivel = '$nivel'";
	$resultado5 = $mysqli->query($query5);

	$pdf->Cell(180,6,' ',0,1, 'C');
	$pdf->Cell(180,10,utf8_decode('ESTILO DE APRENDIZAJE'),1,1, 'C', 1);
	$pdf->Cell(90,6,utf8_decode('Tipo de estilo de aprendizaje'),1,0, 'C',1);
	$pdf->Cell(90,6,'Porcentaje general',1,1,'C',1);

	while($row5 = $resultado5->fetch_assoc())
	{
        $pdf->Cell(90,6,'Estilo Activo',1,0,'R');
	...
	{
		$pdf->Cell(90,6,'Estilo Reflexivo',1,0,'R');
	...
	{
		$pdf->Cell(90,6,utf8_decode('Estilo Teórico'),1,0,'R');
	...
	{
		$pdf->Cell(90,6,utf8_decode('Estilo Pragmático'),1,0,'R');
	...
	WHERE C.Fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' AND U.Nivel = '$nivel'";
	$resultado9 = $mysqli->query($query9);

	$pdf->Cell(180,6,' ',0,1, 'C');
	$pdf->Cell(180,10,utf8_decode('CANAL DE APRENDIZAJE'),1,1, 'C', 1);
	$pdf->Cell(90,6,utf8_decode('Tipo de canal de aprendizaje'),1,0, 'C',1);
	$pdf->Cell(90,6,'Porcentaje general',1,1,'C',1);

	while($row9 = $resultado9->fetch_assoc())
	{
        $pdf->Cell(90,6,'Canal Visual',1,0,'R');
	...
	{
		$pdf->Cell(90,6,'Canal Auditivo',1,0,'R');
	...
	{
		$pdf->Cell(90,6,utf8_decode('Canal Kinestésico'),1,0,'R');
